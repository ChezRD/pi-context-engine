...
export function foldInstructions(config: ExtensionConfig): string {
	return [
		"DeepSeek cache fold: preserve current task state, file decisions, constraints, and recent turns. Summarize older completed work compactly.",
		`Preferred summary model/profile: ${config.foldSummaryModel}.`,
	].join("\n");
}

export function compactOptions(config: ExtensionConfig, _ctx: any): Record<string, unknown> {
	return { customInstructions: config.autoFold ? foldInstructions(config) : undefined };
}
